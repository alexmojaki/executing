...
class App:
    def inspect_opcode(self, bytecode, index, lineno):
        ...
        try:
            ...
            return result
        except (KnownIssue,VerifierFailure,NotOneValueFound,AssertionError,RuntimeError) as e:
    ...
    def dump_deadcode(self, filename):
        ...
        def report(node, tree):
            ...
            t = tree.add("%s %s" % (name, deadcode) + (" %s:%s"%(node.lineno,end_lineno(node)) if hasattr(node,"lineno") else ""))
            dots = False
            ...
            for child in ast.iter_child_nodes(node):
                if (
                    ...
                    or self.start > end_lineno(child))
                ):
                    if not dots:
                        tree.add("...")
                        dots = True
